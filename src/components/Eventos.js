import React, { Component }from 'react';



export class EventosES6 extends Component {
    constructor(props){
        super(props);

        this.state = {
            contador: 0
        }

        this.sumar = this.sumar.bind(this);
        this.restar = this.restar.bind(this);
    }

    sumar(e){
        this.setState ({
            contador: this.state.contador + 1
        })

        console.log(this);
    }

    restar(e){
        this.setState({
            contador: this.state.contador -1
        })

        console.log(this);

    }

    render() {
        return (
            <div>
                <h2> Eventos en componente de clase ES6</h2>
                <nav>
                    <button onClick = {this.sumar}>+</button>
                    <button onClick = {this.restar}>-</button>
                </nav>
                <h3> {this.state.contador}</h3>
               
            </div>
        );
    }
}

// Las property initializers.
export class EventosES7 extends Component {
 

        state = {
            contador: 0
        }

    
// Arrows Funtions
    sumar = (e) =>{
        this.setState ({
            contador: this.state.contador + 1
        })

        console.log(this);
    }

    restar = (e) =>{
        this.setState({
            contador: this.state.contador -1
        })

        console.log(this);

    }

    render() {
        return (
            <div>
                <h2> Eventos en componente de clase ES7</h2>
                <nav>
                    <button onClick = {this.sumar}>+</button>
                    <button onClick = {this.restar}>-</button>
                </nav>
                <h3> {this.state.contador}</h3>
               
            </div>
        );
    }
}

/*
function Boton(props) {
    return (
        <button onClick={ props.myOnClick } >Boton hecho componete</button>
    )
}
*/

function Boton({myOnClick}) {
    return (
        <button onClick={myOnClick } >Boton hecho componete</button>
    )
}

export class MasSobreEventos extends Component {
    constructor(props){
        super(props);
    }

    handleClick = (e, msg) =>{
        console.log(e);        
        console.log(e.target);

        console.log(e.nativeEvent);
        console.log(e.nativeEvent.target);

        console.log(msg);

    }

    render() {
        return (
            <div>
               <h2> Mas Sobre Eventos</h2> 
               <button onClick = {(e) => this.handleClick(e, "Hola pasando parametro desde un evento") } >Saludar</button>

                { /* Esto no funciona por que es un componente de react.

               <Boton 
                    onClick = {(e) => 
                    this.handleClick(e, "Hola pasando parametro desde un evento")}  />

                */}

               { /* Eventos Personalizados */ }
               <Boton myOnClick = {(e) => this.handleClick(e, "Hola pasando parametro desde un evento")} />
            </div>
        )
    }
}